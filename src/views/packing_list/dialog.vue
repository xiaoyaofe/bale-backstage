  ...
  methods: {
    ...
    determineAddPackage() {      console.log('确定添加打包任务');
      if (Object.values(this.addPackageInfo).every(v => !!v)) {
        ...
        this.$message({ type: 'warning', message: `新建打包任务参数不能为空` });
      }

    },
    // 取消筛选打包任务
    cacelFilterPackaging() {      console.log('取消筛选打包任务');
    ...
    // 确定筛选打包任务
    determineFilterPackaging() {      console.log('确定筛选打包任务');
      this.queryPackageList()
    },
    // 
    queryPackageList() {
