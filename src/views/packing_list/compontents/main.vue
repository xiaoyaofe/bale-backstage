    ...
    >
      ...
      <el-table-column label="操作" width="260" align="center" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button type="success" plain @click="startPacking(scope.$index, scope.row)">打包</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      tableData: [],            //表格数据
      tableHead: ['任务ID', '任务名称', '应用ID', '应用名称', '渠道包ID', '渠道包名称', '母包ID', '母包名称', '渠道', 'sdk标识', '证书ID', '证书名称', '任务状态', '构建时间'],
    }
  },
  computed: {
    // 加载应用列表
  ...
  methods: {
    ...
    getHistory(index, row) {
      this.$router.push({ path: 'historyPage', query: { index: index, taskId: this.packing_list.tableData.data[index].taskId, index } })
    },
    // 打包
    startPacking(index, row) {
      this.$confirm('即将开始执行打包任务, 是否继续?', '提示', {
        ...
        this.$store.dispatch('testPackageParams', params).then((data) => {
          this.$store.dispatch('performPackagingTask', params).then((data) => {
            this.isLoading = false;
            return Vue.prototype.$message({ message: '打包成功', type: 'success', duration: 1500 })
